...
export class MongoDB implements Database{
    ...
    async updateNameProject(projectId, newNameProject){
        try{
            const result= await Project.updateOne({ _id: projectId }, { name: newNameProject});
            console.log('Nome modificato con successo:', result);
        }catch(err){
            console.error('Errore durante la modifica del nome del progetto:'+projectId, err);
        }
    }

    async setEndDate(projectId){
        try{
            const result= await Project.updateOne({_id:projectId}, {endDate: Date.now});
            console.log('Aggiunta data di fine: ', result);
        }catch(err){
            console.error('Errore durante l\'aggiunta della data di fine:'+projectId, err);
        }
    }
    ...
    async updateNameUser(idUser, newNameUser ){
        try{
            ...
            console.error('Errore durante la modifica del nome dell\'utente:'+idUser, err);
        }
    }

    async updateEmailUser(idUser, newEmailUser ){
        try{
            const result= await User.updateOne({ _id: idUser }, { mail: newEmailUser});
            console.log('Mail modificata con successo:', result);
        }catch(err){
            console.error('Errore durante la modifica della mail dell\'utente:'+idUser, err);
        }
    }
    
    async updatPasswordUser(idUser, newPasswordUser ){
        try{
            const result= await User.updateOne({ _id: idUser }, { password: newPasswordUser});
            console.log('Password modificato con successo:', result);
        }catch(err){
            console.error('Errore durante la modifica della password dell\'utente:'+idUser, err);
        }
    }

    async updatRoleUser(idUser, newRoleUser ){
        try{
            const result= await User.updateOne({ _id: idUser }, { role: newRoleUser});
            console.log('Ruolo modificato con successo:', result);
        }catch(err){
            console.error('Errore durante la modifica del ruolo dell\'utente:' +idUser, err);
        }
    }
    ...
    checkIfUserExists(userId) {
        try {
          ...
          console.error('Errore durante la verifica dell\'esistenza del\'utente:', error);
          return false;
        }
      }
      ...
      async addProjectToUser(userId, projectId) {
        ...
        if( this.checkIfProjectExists(projectId)){
          // Aggiungi il progetto all'array dei progetti dell'utente
          ...
          console.error('Il progetto specificato non esiste nel database o si è verificato un errore durante la ricerca.');
        }
    }
    ...
    async delateProjectToUser(userId, projectId) {
        try {
            ...
            user.project= user.project.filter(project => project.toString() !== projectId);
            ...
            console.error('Errore durante la rimozione del progetto dall\'utente:', error);
          }
    }
    ...
    async delateUserStoryToUser(userId, userStoryId) {
        try {
            ...
            // Rimuovi il progetto dall'array dei progetti dell'utente
            user.userStory= user.userStory.filter(userStory => userStory.toString() !== userStoryId);
    ...
    async updateTitleEpicStory(epicStoryId, newTitle){
        try{
            const result= EpicStory.updateOne({ _id: epicStoryId }, { title: newTitle});
            console.log('Titolo modificato con successo:', result);
        }catch(err){
            console.error('Errore durante la modifica del titolo dell\'epic story:'+epicStoryId, err);
        }
    }
    
    async updatedescriptionEpicStory(epicStoryId, newDespription){
        try{
            const result= EpicStory.updateOne({ _id: epicStoryId }, { descript: newDespription});
            console.log('Descrizione modificato con successo:', result);
        }catch(err){
            console.error('Errore durante la modifica della descrizione dell\'epic story:'+epicStoryId, err);
        }
    }
    ...
    checkIfEpicStoryExists(epicStoryId) {
        try {
          ...
          console.error('Errore durante la verifica dell\'esistenza del\'epic story:', error);
          return false;
        }
      }
    ...
    async delateProjectToEpicStory( epicStoryId) {
        try {
            ...
            console.error('Errore durante la rimozione del progetto dall\'epic story:', error);
          }
    }
    ...
    async delateUserStoryToEpicStory( epicStoryId, userStoryId) {
      try {
          ...
          // Rimuovi l'utente dall'array degli utente del progetto
          epicStory.userStory= epicStory.userStory.filter(userStory => userStory.toString() !== userStoryId);
      
          // Salva le modifiche
          await epicStory.save();
      
          console.log('L\'user story e\' stata rimossa con successo dall\'epic story:', epicStory);
        } catch (error) {
          console.error('Errore durante la rimozione dell\'user story dall\'epic story:', error);
        }
    }
    //Method User story
    checkIfUserStoryExists(userStoryId) {
        try {
          ...
          console.error('Errore durante la verifica dell\'esistenza del\'user story:', error);
          return false;
        }
      }
}