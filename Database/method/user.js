...
function updateNameUser(idUser, newNameUser ){
    User.updateOne({ _id: idUser }, { name: newNameUser}, (err, result) => {
        if (err) {
          console.error('Errore durante la modifica del nome dell\'utente:'+this._id, err);
        } else {
...
function updateEmailUser(idUser, newEmailUser ){
    User.updateOne({ _id: idUser }, { mail: newEmailUser}, (err, result) => {
        if (err) {
          console.error('Errore durante la modifica della mail dell\'utente:'+this._id, err);
        } else {
...
function updatPasswordUser(idUser, newPasswordUser ){
    User.updateOne({ _id: idUser }, { password: newPasswordUser}, (err, result) => {
        if (err) {
          console.error('Errore durante la modifica della password dell\'utente:'+this._id, err);
        } else {
...
function updatRoleUser(idUser, newRoleUser ){
    User.updateOne({ _id: idUser }, { role: newRoleUser}, (err, result) => {
        if (err) {
          console.error('Errore durante la modifica del ruolo dell\'utente:' +this._id, err);
        } else {
...
module.exports = { createUser, delateUser, delateFirstUser, updateNameUser, updatPasswordUser, updateEmailUser, updatRoleUser, addProjectToUser, delateProjectToUser, checkIfUserExists, delateUserStoryToUser, addUserStoryToUser };
